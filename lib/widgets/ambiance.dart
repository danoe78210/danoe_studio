...
class CandleGlow extends StatefulWidget {
  ...
  final double size;
  const CandleGlow({super.key, this.alignment = Alignment.center, this.size = 500});
...
class _CandleGlowState extends State<CandleGlow> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(animation: _c, builder: (_, __) {
      ...
      return Align(alignment: widget.alignment,
        child: Transform.scale(scale: scale,
          child: Container(width: widget.size, height: widget.size,
            decoration: BoxDecoration(shape: BoxShape.circle,
              gradient: RadialGradient(colors: [
                AntiqueTheme.candleGlow.withOpacity(opacity),
                AntiqueTheme.candleGlow.withOpacity(0),
              ])))));
    });
  }
}
...
class _DustParticlesState extends State<DustParticles> with SingleTickerProviderStateMixin {
  late final AnimationController _c =
  ...
  Widget build(BuildContext context) {
    return SizedBox.expand(
      child: AnimatedBuilder(animation: _c,
        builder: (_, __) => CustomPaint(painter: _DustPainter(_c.value, _ps))),
    );
  }
}
...
class _P {
  ...
  factory _P.rand(int i) {
    ...
    return _P(r.nextDouble(), 0.3 + r.nextDouble() * 0.7, r.nextDouble(), 1.0 + r.nextDouble() * 1.8);
  }
}
...
class _DustPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    for (final p in ps) {
      ...
      final x = p.baseX * s.width + math.sin((prog * 6 + p.phase) * math.pi) * 14;
      final fade = math.sin(prog * math.pi);
      c.drawCircle(Offset(x, y), p.size,
          Paint()..color = AntiqueTheme.agedGold.withOpacity(0.25 * fade));
    }
  }
...
class Vignette extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(decoration: const BoxDecoration(
      gradient: RadialGradient(center: Alignment.center, radius: 0.95,
        colors: [Color(0x00000000), Color(0x99000000)], stops: [0.6, 1.0])));
  }
}
