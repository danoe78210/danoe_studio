...
def generer(safe=False):
    ...
    for s in segments:
        ...
        pages = PdfReader(buf)
        for p in pages.pages: writer.add_page(p)
    ...
    nom = os.path.join(BASE, slug(titre) + '_KDP.pdf')
    with open(nom, 'wb') as f: writer.write(f)
